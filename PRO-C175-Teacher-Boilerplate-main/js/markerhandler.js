...
AFRAME.registerComponent("markerhandler", {
  init: async function () {
    ...
    this.el.addEventListener("markerFound", () => {
      ...
      //changing compound visibilty
      compounds[barcodeValue]["compounds"].map(item => {
        var compound = document.querySelector(`#${item.compounds_name}-${barcodeValue}`);
  ...
  tick: function () {
    if(elementsArray.length > 1){
      ...
      if(length === 2){
        ...
        var marker1 = this.document.querySelector(`#marker-${elementsArray[0].barcode_value}`);

        var marker2 = this.document.querySelector(`#marker-${elementsArray[1].barcode_value}`);
  ...
  getDistance: function (elA, elB) {
    ...
    return e1A.object3D.position.distanceTo(e1b.object3D.position);
